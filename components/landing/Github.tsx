        ...
        if (data?.contributions && Array.isArray(data.contributions)) {
          ...
          const validContributions = data.contributions
            ...
            .map((item: any) => ({
              ...
              level: Number(item.intensity || 0) as ContributionItem["level"],
            }));
    ...
    <Container className="mt-20">
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          {githubConfig.wakatime.enabled && (
            <div className="flex items-center gap-2 text-sm text-neutral-400">
              <span className="flex items-center gap-1">
                {githubConfig.wakatime.status}
              </span>
              <Box className="w-4 h-4" />
              <span>
                {githubConfig.wakatime.text}{" "}
                <span className="text-foreground font-medium">
                  {githubConfig.wakatime.time}
                </span>
              </span>
            </div>
          )}
